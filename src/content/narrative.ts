export type NarrativePanel = { kicker: string; heading: string; body: string };

/** The "problem → call → fix" story for the pinned rising-water section. */
export const narrative: NarrativePanel[] = [
  {
    kicker: "The problem",
    heading: "It always starts small.",
    body: "A slow drain. A patch of damp. A water bill that crept up. The little things are the warning, and they never fix themselves.",
  },
  {
    kicker: "The call",
    heading: "One call, a real plumber.",
    body: "No call centre, no runaround. You talk to someone who knows the job and can be at your door, often the same day.",
  },
  {
    kicker: "The fix",
    heading: "Done once. Done right.",
    body: "A fixed price agreed before we start, quality parts, and a 12-month workmanship warranty that means it stays fixed.",
  },
];
