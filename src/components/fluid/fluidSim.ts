...
export type FluidOptions = {
  ...
  shouldSuppressInject?: (e: MouseEvent) => boolean;
...
export function createFluidSim(canvas: HTMLCanvasElement, opts: FluidOptions = {}): FluidHandle {
  ...
  // `inject=false` updates the tracked position WITHOUT requesting a splat — so
  // the pointer can pass over UI (e.g. the nav) without stirring the water, and
  // there's no velocity jump when it crosses back onto the canvas.
  function move(clientX: number, clientY: number, inject: boolean) {
    ...
    pointer.x = x;
    pointer.y = y;
    if (inject) pointer.moved = true;
  }
  const onMouse = (e: MouseEvent) => {
    if (e.clientY >= window.innerHeight) return;
    const inject = !o.shouldSuppressInject || !o.shouldSuppressInject(e);
    move(e.clientX, e.clientY, inject);
  };
